...
final class WatchlistTests: XCTestCase {
    ...
    func testWatchlistViewModel() throws {
        let fetcher = MockWatchlistFetcher()
        let viewModel = WatchlistViewModel(siteURLs: [URL(string: "https://en.wikipedia.org")!,
                                                      URL(string: "https://es.wikipedia.org")!], fetcher: fetcher)
        viewModel.fetchWatchlist { response in
            switch response {
            ...
            case .failure(let error):
                XCTFail("Unexpected WatchlistViewModel failure: \(error)")
            }
